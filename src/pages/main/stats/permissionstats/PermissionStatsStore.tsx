...
interface PermissionStatsStoreType extends BaseStoreState<any> {
  year: number | null;
  month: number;
...
export const PermissionStatsStore = create<PermissionStatsStoreType>((set) => ({
  page: 1,
  search: "",
  loading: false,
  data: null,
  ...
  async fetchData(id: number) {
    const { month, year } = PermissionStatsStore.getState();
    const data = {
      departmentMemberID: id ? id : 0,
      month,
      year,
    };
    const result = await apiProvider.fetchPermissionStatsData(data);
    set({ loading: true });
    if (result != null) {
